...
The automorphism distribution by cubes can be summarized in the bar-plot grphic

```{r barplot, fig.height = 4, fig.width = 4}
cts <- unlist(brca1_autm_range)
counts <- table(cts$cube)

par(family = "serif", cex = 0.8, font = 2, las = 1)
...
```{r trasnl}
brca1_aln_aa <- translate(brca1_aln@unmasked)
brca1_aln_aa
```


```{r subs}
brca1_aln_aa_region <- AAStringSet(brca1_aln_aa, start = 37, end = 42)
brca1_aln_aa_region
```


```{r seqlogo_aa}
ggseqlogo( as.character(brca1_aln_aa_region) )
```
