...
author: 
    - name: Robersy Sanchez
      affiliation: >
          EpiMethyl Analytics, 10401 Baur Blvd, Suite F, St. Louis, MO 63132
      email: rsanchez@epimethyl.com
...
This is an R package to compute the automorphisms between pairwise aligned DNA
...
regions from a population (from any species or closely related species) can be
...
the direct sum of Abelian groups of _prime-power order_:
...
You can install the `GenomAutomorphism` package from GitHub as follows

```{r inst-git, eval=FALSE}
BiocManager::install('genomaths/GenomAutomorphism')
```

You can install the `GenomAutomorphism` 'beta' package with the latest
updates (which may differ from the Bioconductor version) from GitHub as
follows:
...
Herein, automorphisms are considered algebraic descriptions of mutational
events observed in codon sequences represented on different Abelian groups. In
...
For further support about the symmetric group on the 24 Abelian groups of 
genetic-code cubes, users can also see 
[Symmetric Group of the Genetic-Code Cubes.](https://is.gd/pTl8Js), 
...
## Read the _FASTA_ alignment and encode the sequences

A pairwise sequence alignment of protein-coding regions of SARS coronavirus GZ02
...
Group operations defined on the sets of DNA bases and codons are associated with
physicochemical and/or biophysical relationships between DNA bases and between
...
physicochemical and/or biophysical relationships between the set’s elements.
...
physicochemical and/or biophysical relationships as group operations between
...
representation of base-triplets on the Galois field *GF(4)* (or in *GF(5)*)
leads to a genetic-code vector 3D-space, which is mathematically equivalent to
a cube inserted in the 3D space (1). Each cube is denoted according to the 
...
($\mathbb{Z}_{4}$). In practical terms, this is equivalent to replacing each
DNA base by the corresponding integer element. The base replacement in cube
"ACGT" and group "Z4" ($\mathbb{Z}_{4}$) is:
```{r int}
...
(left) are returned, as well as the coordinate representation on
$\mathbb{Z}_{64}$ (_coord1_ and _coord2_). 

## _"Dual"_ genetic-code cubes

Of particular interest is the coordinate representation on _"dual"_
...
Their codon representations on $\mathbb{Z}_{64}$ are:
```{r codons}
## cube ACGT
...
that we can work only with genomic regions of interest by giving the
_start_ and _end_ alignment coordinates. In $\mathbb{Z}_{64}$ automorphisms 
are described as functions $f(x) = A x \ mod \ \mathbb{Z}_{64}$, where $A$ and $x$ are
elements from the set of integers modulo 64. Below, in function
[automorphisms](https://is.gd/WMxUKz) three important arguments are given
values: _group = "Z64"_, _cube = c("ACGT", "TGCA")_, and _cube_alt = c("CATG",
"GTAC")_. 

In groups $\mathbb{Z}_{64}$ and $\mathbb{Z}_{125}$, not all the mutational events can be described as
...
denoting pairs of _dual_ genetic-code cubes is given as the argument for 
_cube_. The _group_ setting specifies on which group the automorphisms will
be computed. These groups can be: $\mathbb{Z}_{5}$ (`group = "Z5"`), $\mathbb{Z}_{64}$ (`group = "Z64"`), $\mathbb{Z}_{125}$ (`group = "Z125"`), and $\mathbb{Z}_{5}^3$ (`group = "Z5^3"`).

If automorphisms are not found in the first set of dual cubes, then the 
algorithm searches for automorphisms in an alternative set of dual cubes.
...
The last result can be summarized by gene regions as follows:
```{r range}
...
mutational events observed in it, quantitatively represented as automorphisms 
on genetic-code cubes.

Searching for automorphisms on $\mathbb{Z}_{64}$ allows a quantitative
...
Next, the automorphisms for the whole pairwise alignment of the SARS-CoV-2
...
Regions not described by automorphisms can be described as translations
(labeled "Trnl") and they can be shown as follows:
```{r non-aut}
...
The whole regions can be summarized by typing:
```{r range_3}
...
mutational events, four regions with two _indel_ mutations, and one region
with three _indel_ mutations.
...
In general, _indel_ mutational events can be modeled as translations on
$\mathbb{Z}_{64}$.

## Bar plot of automorphism distribution by cubes
The automorphism distribution by cubes can be summarized in the following
bar plot:
...
## Grouping automorphisms by automorphism coefficients. Types of mutations
...
Barplot of the frequency of mutation types greater than 2.
...
Every single base mutational event across the MSA was classified according to
IUPAC nomenclature: 1) According to the number of hydrogen bonds (on DNA/RNA
double helix): strong S={C, G} (three hydrogen bonds) and weak W={A, T} (two
hydrogen bonds). 2) According to the chemical type: purines R={A, G} and
pyrimidines Y={C, T}. 3) According to the presence of amino or keto groups on
the base rings: amino M={C, A} and keto K={G, T}. Insertion-deletion 
mutational events are represented by symbol '---'. Constant (hold) base
positions were labeled with letter H. So, codon positions labeled as HKH means
that the first and third bases remain constant and mutational events between
...
where the $\alpha_i$ are known as initial concentration parameters, which represent our initial belief or assumption about the distribution of the mutation types, and $n_i$ stands for the observed frequencies:
```{r freq-table}
...
bat-SL-CoVZC45) by mutation types is shown in the last figure. Results are
...
```{r prob, fig.height = 12, fig.width = 14, warning=FALSE}
probs <- post_prob$Posterior_Probability
names(probs) <- post_prob$Codon

par(family = "serif", cex.axis = 2, font = 2, las = 1, 
    cex.main = 2.1, mar = c(7,2,4,2))
barplot(probs, main="Automorphism Probability per Mutation type",
        col = colorRampPalette(c("red", "yellow", "blue"))(36), 
...
### Conserved regions
Conserved and non-conserved gene regions can be easily observed in most 
MSA-editing bioinformatic tools. However, here we are interested in obtaining
the region coordinates for further downstream analysis.

Conserved regions from pairwise comparisons are obtained with the
`conserved_regions` function:
```{r conserved_regions}
...
Several regions are shared by more than one comparison.
```{r uniq}
...
Alternatively, we can use the algebraic representation on 
$\mathbb{Z}_{125}$.
...
([4](#4)). Each codon is represented by a coordinate in the 3D space.

Automorphisms are described as functions $f(x) = A x \ mod \ \mathbb{Z}^3_5$,
where $A$ is a diagonal matrix with elements in $\mathbb{Z}_5$.
...
The result is available with the package
```{r autm_3d}
data(autm_3d, package = "GenomAutomorphism")
autm_3d
```

## Grouping automorphisms by automorphism coefficients

Automorphisms that preserve codons (DNA base-triplets) are represented by the
...
The whole-genome mutational events are represented as automorphisms on the 
3D space $\mathbb{Z}_{5}^3$, specifically on the cube ACGT (see [4](#4)).
