...
conserved_regions:
```{r conserved_regions}
conserv <- conserved_regions(autm)
conserv
```
...
```{r non-consv}
non_conserv <- conserved_regions(autm, conserved = FALSE)
non_conserv
```

Several regions are similar for more than one comparison.
```{r uniq}
conserv_unique <- conserved_regions(autm, output = "unique")
conserv_unique
```

Unique non-conserved pairwise regions:
```{r uniq-non-consr}
non_conserv_unique <- conserved_regions(autm, conserved = FALSE, output = "unique")
non_conserv_unique
```
...
```{r barplot_2, fig.height = 4, fig.width = 3.2}
