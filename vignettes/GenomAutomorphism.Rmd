...
The FASTA file carries the pairwise sequence alignment of protein coding regions
SARS coronavirus GZ02 (GenBank: AY390556.1) and Bat SARS-like coronavirus
isolate Rs7327 (GenBank: KY417151.1).
...
```{r barplot, fig.height = 4, fig.width = 7}
...
mtext(side = 1,line = -1.5, at = c(0.7, 1.9, 3.1, 4.3, 5.5),
      ...
      col = c("white","red", "black", "yellow", "orange"))
...
```{r non-consv}
non_conserv <- conserved_regions(covid_autm, conserved = FALSE)
non_conserv
```
...
```{r uniq-non-consr}
non_conserv_unique <- conserved_regions(covid_autm, conserved = FALSE, output = "unique")
non_conserv_unique
```
...
barplot(counts, main="Automorphism distribution",
        ...
        ylab="Fixed mutational events",
        col=c("darkblue","red"), 
        ylim = c(0, 3000), border = NA, axes = T)
axis(2, at = c(0, 500, 1000, 1500, 2000, 2500, 3000))
