...
control <- partykit::ctree_control(testtype = "MonteCarlo",
                         splittest = TRUE, minsplit = 70, alpha = 0.05)

tree <- partykit::ctree(species ~., data = dt, 
                        teststat = "quadratic",
                        control = control,
...
```{r util-info}
pvals <- unlist(nodeapply(tree, ids = nodeids(tree), 
...
# datos$mut_type <- case_when(substring(datos$mut_type,1,2) == "HH" ~ "HHX",
#                             grepl("H[KMRYSW]H", datos$mut_type) ~ 'HXH',
#                             substring(datos$mut_type,2,3) == "HH" ~ 'XHH')

datos$mut_type <- as.character(datos$mut_type)
datos$mut_type[ which(datos$cube == "Trnl") ] <- "---"
...
datos <- datos[, c( "regions","autms", "mut_type", "cube", "species")]
datos
```

Fitting a classification tree with CHAID algorithm
```{r chaid}
ctrl <- chaid_control(minsplit = 200, minprob = 0.51, alpha2 = 0.01, alpha4 = 0.01)
chaid_res <- chaid(species ~ regions + autms + mut_type + cube , data = datos,
                   control = ctrl)
```



```{r ch, fig.height = 12, fig.width = 24}
plot(chaid_res, margins = c(8, 0, 0, 0),
     terminal_panel = partykit::node_barplot(chaid_res, beside = F, id = F),
    tp_args = list(rot = 50, just = c("right", "top"),ep_args = list(justmin = 15)),
    tnex = 2)
...
dat <- dp[, c("regions", "autms", "mut_type", "cube")]
...
Plotting the tree with [ggparty](https://rdrr.io/cran/ggparty/man/ggparty.html)
```{r chaid-tree, fig.height = 18, fig.width = 26}
ggparty(chaid_tree) +
    geom_edge(aes(color = id, size = node.size[id]/300), show.legend = FALSE) +
    geom_edge_label(size = 5, colour = "red",
                    fontface = "bold", 
                    shift = 0.64, 
                    nudge_x = -0.01,
                    max_length = 10,
    ...
    geom_node_label(line_list = list(aes(label = paste0("Node ", id, ": ", splitvar)),
                aes(label = paste0("N=", node.size[id], ", p", 
                    ...
                    line_gpar = list(list(size = 12), 
                                     list(size = 11)),
                ids = "inner", fontface = "bold", size = 11) +
    geom_node_info() +
