...
#' @aliases base2codon
#' @export
setMethod("base2codon", signature(x = "character"),
    function(x) {
        nch <- nchar(x)
        if ((nch %% 3) != 0) {
            n <- (3 - nch %% 3)
            ...
            x <- paste0(x, n)
            warning("*** Base sequence of 'x' is not multiple of 3. ",
...
#' @aliases base2codon
#' @export
setMethod("base2codon", signature(x = "DNAStringSet"),
    function(x) {
        x <- as.character(x)      
...
#' @aliases base2codon
#' @export
setMethod("base2codon", signature(x = "DNAMultipleAlignment"),
    function(x) {
        x <- x@unmasked
        x <- base2codon(x)
        return(x)
